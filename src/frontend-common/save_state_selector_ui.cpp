...
void SaveStateSelectorUI::InitializeListEntry(ListEntry* li, CommonHostInterface::ExtendedSaveStateInfo* ssi)
{
  ...
  if (ssi && !ssi->screenshot_data.empty())
  {
    li->preview_texture = m_host_interface->GetDisplay()->CreateTexture(
      ssi->screenshot_width, ssi->screenshot_height, HostDisplayTexture::Format::RGBA8, ssi->screenshot_data.data(),
      sizeof(u32) * ssi->screenshot_width, false);
  }
  else
  {
    li->preview_texture = m_host_interface->GetDisplay()->CreateTexture(
      PLACEHOLDER_ICON_WIDTH, PLACEHOLDER_ICON_HEIGHT, HostDisplayTexture::Format::RGBA8, PLACEHOLDER_ICON_DATA,
